...
const config =
  // TODO - Prefered to confine this to test file when possible.
  process.env.NODE_ENV === 'test'
    ? (testConfig as PlaylistExporter.IParseArgs) // skip parsing in test mode
    : parse<PlaylistExporter.IParseArgs>(
        {
          host: {
            type: String,
            optional: true,
          ...
          envFile: {
            ...
            group: 'credentials',
            description:
              'If you want to use an environment file to pass credentials/configuration varables, ' + 'specify the path to the file here.',
          },
          playlistId: {
            type: String,
            optional: true,
          ...
          playlistOnly: {
            ...
            optional: true,
            defaultValue: false,
            description: 'If set, this will export the playlist file only and no music tracks.\n' + `${ansiColors.dim('Default: false')}`,
          },
          absolutePaths: {
            type: Boolean,
            alias: 'a',
            ...
            defaultValue: false,
            description:
              'Determines wether song paths in the playlist should be absolute or relative, in relation to the playlist location.' +
              'If you are using this tool to backup playlists and not songs, this would typically be absolute.' +
              `${ansiColors.dim('Default: false')}`,
          },
          list: {
            type: Boolean,
          ...
          verbose: {
            ...
            alias: 'v',
            defaultValue: false,
            description: 'Enables more detailed information about the program function.\n' + `${ansiColors.dim('Default: false')}`,
          },
          zuneCompatibility: {
            type: Boolean,
            optional: true,
            ...
            defaultValue: false,
            description:
              'If true, forces Zune-compatible output for paths and tags.\n' +
              'Tracks are saved in the export path using a unique ID as the filename, without artist or album subdirectories.\n' +
              'ID3v2.4 tags are stripped and replaced with Zune-compatible ID3v2.3 tags.\n' +
              `${ansiColors.dim('Default: false')}`,
          },
...
export const progresBarConfig = {
  format: `Exporting '{name}' | ${ansiColors.cyanBright(
    '{bar}',
  )} | {value}/{total} [{skipped} skipped, {failed} failed] | {track} [{size}Mb]`,
  barCompleteChar: '\u2588',
  barIncompleteChar: '\u2591',
  hideCursor: true,
};
